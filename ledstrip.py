...
class strand:
    ...
    def __init__(self, leds=22, dev="/dev/spidev0.1"):
        #Variables:
        #leds -- strand size
        #dev -- spi device
        self.leds = leds
    ...
    def fill(self, r, g, b, start=10, end=0):
    ...
    def even(self, r, g, b):
        #Fill the even leds with a single color
        for led in range(10, self.leds)[2::2]:
            self.buffer[led][0] = self.gamma[g]
            self.buffer[led][1] = self.gamma[r]
            self.buffer[led][2] = self.gamma[b]
        #self.update()
    ...
    def odd(self, r, g, b):
        #Fill the odd leds with a single color
        for led in range(10, self.leds)[1::2]:
            self.buffer[led][0] = self.gamma[g]
            self.buffer[led][1] = self.gamma[r]
            self.buffer[led][2] = self.gamma[b]
        #self.update()
